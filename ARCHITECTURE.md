...
Both the Pi and Claude Code installed integrations expose channel membership changes only as user-invoked commands. Neither integration registers an LLM-callable subscribe or unsubscribe tool, and neither subscribes automatically. The installed Claude Code `/inter-agent` skill exposes `subscribe`, `unsubscribe`, and `publish` as user-invoked commands through the bundled wrapper, using the active listener's connected routing name for membership and publishing. It also exposes read-only `channels` diagnostics only on explicit user request; diagnostics use a short-lived authenticated server connection and do not require or change the active listener.
