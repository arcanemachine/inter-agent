...
Gitlinks identify the exact tested source set. `COMPATIBILITY.md` records the compatible semantic versions; the Gitlinks, not README prose, identify source revisions. Updating a Gitlink requires child package gates, cross-repository acceptance, and a compatibility update when the supported version set changes.
