...
8c. **Pi mailbox continuity across extension reload** — preserve the bounded unread mailbox through same-process `/reload` with a one-use in-memory handoff, while terminated-session resume and true replacement remain empty. **Status: active; packet prepared in `.agents/plans/pi-mailbox-reload-continuity/01-one-use-reload-handoff.md`.**
