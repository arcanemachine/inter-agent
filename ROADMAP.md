...
9. **Installed cross-adapter TLS acceptance** — prove Pi and Claude helper/listener paths interoperate over `wss://`, including pub/sub, reconnect, certificate failures, and config propagation. **Status: active; packet prepared in `.agents/plans/installed-cross-adapter-tls-acceptance/01-installed-wss-matrix.md`.**
