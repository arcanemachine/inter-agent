...
7a. **Pi startup inter-agent identity** — add `pi --inter-agent <name>` so an installed Pi worker connects under an explicit routing name through the existing listener path, including session replacement/reload and bounded nonfatal failures. **Status: active; packet prepared in `.agents/plans/pi-startup-identity/01-inter-agent-flag.md`.**
8. **Pi queued mailbox** — queue direct, broadcast, and channel message bodies by default; add explicit immediate mode, metadata-only notices, bounded in-memory storage, and selected/all message reads. **Status: next after priority insertion 7a; no active packet prepared.**
...
Next activation step: complete active priority insertion 7a, then prepare the closeout queue item 8 packet and obtain explicit dispatch authorization.
