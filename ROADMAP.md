...
7. **Claude Code sandbox connect failure** — reproduce and resolve the installed `/inter-agent connect` exit-127 failure recorded for the interline sandbox, or document a verified environment constraint. **Status: active; packet linked from `.agents/PLAN.md`.**
...
Next activation step: follow closeout queue item 7, Claude Code sandbox connect failure.
