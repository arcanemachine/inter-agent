...
Inbound inter-agent bodies are queued by default so a receiving agent can continue its own reasoning and decide when to read peer messages. Mailbox state lives only in TypeScript extension memory for the current Pi extension session: it survives listener disconnect/reconnect within the same session, and survives a same-process extension `/reload` through a one-use, in-memory handoff that never writes bodies to the transcript, settings, environment, arguments, or disk. It clears on Pi process termination and on real session/process replacement (`new`, `resume`, or `fork`), or `quit`/failed reload; a resumed terminated session starts empty even if its transcript still shows old mailbox notices or read results, so previously shown IDs read as missing.
...
3. **Run these commands in Pi** and confirm each works:
   ...
   - Disconnect and reconnect the listener; unread mailbox state survives within the session. Reload the extension in the same process (`/reload`) and unread mailbox state survives; terminate Pi or replace/resume the session and mailbox state clears, so previously shown IDs read as missing.
