...
@pytest.mark.asyncio
async def test_status_reports_auth_failure(
    monkeypatch: pytest.MonkeyPatch, live_server: LiveServer
) -> None:
    monkeypatch.setenv("INTER_AGENT_SECRET", "wrong-secret")
...
@pytest.mark.asyncio
async def test_status_reports_protocol_mismatch(unused_tcp_port: int) -> None:
    ...
    assert status.reachable is True
    assert status.message == "server returned an invalid status response"


@pytest.mark.asyncio
async def test_status_reports_unreachable_configured_endpoint(
